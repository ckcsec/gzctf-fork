...
import { getGameStatus, toLimitTag, useGame } from '@Hooks/useGame'
import { usePageTitle } from '@Hooks/usePageTitle'
import { useTeams, useUser } from '@Hooks/useUser'
import api, { GameJoinModel, GameParticipationMode, ParticipationStatus } from '@Api'
...
const GameDetail: FC = () => {
  ...
  const isIndividual = game?.participationMode === GameParticipationMode.Individual
  const hasParticipationIdentity = isIndividual || Boolean(teams?.length)

  const canSubmit =
    (status === ParticipationStatus.Unsubmitted || status === ParticipationStatus.Rejected) &&
    isGameOpenForJoin &&
    Boolean(user) &&
    hasParticipationIdentity
    ...
    modals.openConfirmModal({
      ...
      children: (
        <Stack gap="xs">
          ...
          {!isIndividual && <Text size="sm">{t('game.content.leave.content.1')}</Text>}
        </Stack>
      ),
          ...
          <Stack gap={6} className={classes.flexGrowAtSm}>
            ...
            <Stack gap={2}>
              ...
              <Text size="sm" c="dimmed">
                <Trans
                  i18nKey={isIndividual ? 'game.content.joined_status_individual' : 'game.content.joined_status'}
