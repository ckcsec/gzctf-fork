    ...
    IStringLocalizer<Program> localizer) : ControllerBase
{
    ...
    public async Task<IActionResult> JoinGame(int id, [FromBody] GameJoinModel model, CancellationToken token)
    {
        ...
        // Check if user is already in this game through a different participation.
        // Rejected participation can be reused when the player joins again.
        ...
        if (game.ParticipationMode == GameParticipationMode.Individual)
        {
            part = await participationRepository.GetParticipation(user!.Id, id, token);
            team = part?.Team;
        }
        ...
        if (game.ParticipationMode == GameParticipationMode.Individual && team is null)
            team = await teamRepository.CreateIndividualTeam(user!, token);

        // Get existing participation for this team in this game
        part ??= await participationRepository.GetParticipation(team!, game, token);
        ...
        if (game.ParticipationMode == GameParticipationMode.Team
            && game.TeamMemberCountLimit > 0
            && part.Members.Count >= game.TeamMemberCountLimit)
    ...
    public async Task<IActionResult> LeaveGame(int id, CancellationToken token)
    {
        ...
        if (part.Members.Count == 0)
        {
            ...
            if (part.Team.IsIndividual)
                await teamRepository.DeleteTeam(part.Team, token);
        }
        else
            await participationRepository.SaveAsync(token);
