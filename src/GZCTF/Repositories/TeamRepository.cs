...
public class TeamRepository(AppDbContext context) : RepositoryBase(context), ITeamRepository
{
    ...
    public override Task<int> CountAsync(CancellationToken token = default) =>
        Context.Teams.CountAsync(t => !t.IsIndividual, token);

    public Task<bool> CheckIsCaptain(UserInfo user, CancellationToken token = default) =>
        Context.Teams.AnyAsync(t => !t.IsIndividual && t.Captain == user, token);
    ...
    public async Task<Team> CreateIndividualTeam(UserInfo user, CancellationToken token = default)
    {
        Team team = new()
        {
            Name = user.UserName ?? user.Id.ToString("N")[..20],
            Bio = user.Bio[..Math.Min(user.Bio.Length, 72)],
    ...
    public Task<Team[]> SearchTeams(string hint, CancellationToken token = default)
    {
        ...
        var query = int.TryParse(hint, out var id)
            ? Context.Teams.Include(t => t.Members)
                .Where(item => !item.IsIndividual && (item.Name.ToLower().Contains(loweredHint) || item.Id == id))
            : Context.Teams.Include(t => t.Members)
                .Where(item => !item.IsIndividual && item.Name.ToLower().Contains(loweredHint));
    ...
    {
        var team = await Context.Teams.FirstOrDefaultAsync(t => !t.IsIndividual && t.Id == id, token);
