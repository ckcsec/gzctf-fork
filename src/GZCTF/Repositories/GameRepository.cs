    ...
    AppDbContext context) : RepositoryBase(context), IGameRepository
{
    ...
    public async Task<GameJoinCheckInfoModel>
        ...
        new()
        {
            JoinedTeams = game.ParticipationMode == GameParticipationMode.Individual
                ? []
                : await participationRepository.GetJoinedTeams(game, user, token),
    ...
    public async Task<TaskStatus> DeleteGame(Game game, CancellationToken token = default)
    {
        ...
        try
        {
            ...
            var individualTeams = await Context.Teams
                .Where(t => t.IsIndividual && t.Participations.Any(p => p.Game == game))
                .ToArrayAsync(token);
            ...
            Context.RemoveRange(individualTeams);
            Context.Remove(game);
