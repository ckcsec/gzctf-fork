...
public class GameJoinTests(GZCTFApplicationFactory factory)
{
    ...
    [Fact]
    public async Task JoinGame_IndividualMode_WithWrongInviteCode_ShouldNotCreateHiddenTeam()
    {
        var userPassword = "User@Pass123";
        var user = await TestDataSeeder.CreateUserAsync(factory.Services,
            TestDataSeeder.RandomName(), userPassword);
        var game = await TestDataSeeder.CreateGameAsync(factory.Services, "Individual Invite Test Game");
        ...
        using (var scope = factory.Services.CreateScope())
        {
            ...
            gameEntity.ParticipationMode = GameParticipationMode.Individual;
            gameEntity.InviteCode = "INDIVIDUAL_CODE";
            hiddenTeamCount = await context.Teams.CountAsync(t => t.IsIndividual);
        ...
        var joinResponse = await client.PostAsJsonAsync($"/api/Game/{game.Id}",
            new GameJoinModel { TeamId = null, DivisionId = null, InviteCode = "WRONG_CODE" });

        Assert.Equal(HttpStatusCode.BadRequest, joinResponse.StatusCode);

        using var verificationScope = factory.Services.CreateScope();
        var verificationContext = verificationScope.ServiceProvider.GetRequiredService<AppDbContext>();
        Assert.Equal(hiddenTeamCount, await verificationContext.Teams.CountAsync(t => t.IsIndividual));
    }
