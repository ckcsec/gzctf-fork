...
public class AuthenticationTests(GZCTFApplicationFactory factory, ITestOutputHelper output)
{
    ...
    public async Task Account_Login_WithSeededUser_Succeeds()
    {
        ...
        var saved = await context.Users.SingleAsync(user => user.Id == seeded.Id);
        Assert.True(saved.LastSignedInUtc > DateTimeOffset.FromUnixTimeSeconds(0));
    }

    [Fact]
    public async Task Account_Login_WithUnconfirmedUser_ReturnsActivationMessage()
    {
        var password = "Pending!Pass123";
        var seeded = await TestDataSeeder.CreateUserAsync(factory.Services,
            TestDataSeeder.RandomName(),
            password);

        using (var scope = factory.Services.CreateScope())
        {
            var userManager = scope.ServiceProvider.GetRequiredService<UserManager<UserInfo>>();
            var user = await userManager.FindByIdAsync(seeded.Id.ToString());
            Assert.NotNull(user);
        ...
        var error = await loginResponse.Content.ReadFromJsonAsync<RequestResponse>();
        Assert.NotNull(error);
        Assert.Contains("activated", error.Title, StringComparison.OrdinalIgnoreCase);
    }
    ...
    public async Task Register_WithValidData_ReturnsSuccess()
    {
        ...
        using var scope = factory.Services.CreateScope();
        var context = scope.ServiceProvider.GetRequiredService<AppDbContext>();
        var saved = await context.Users.SingleAsync(user => user.UserName == registerModel.userName);
        Assert.True(saved.EmailConfirmed);
        Assert.True(saved.RegisterTimeUtc > DateTimeOffset.FromUnixTimeSeconds(0));
    }
